# MW4 Blender Animation Tools — R21

**Blender 5.1+; add-on version 0.21.0.** R21 tested with Blender 5.1.0; R15 was also tested with 5.2.0. Future Blender versions have not all been tested.
...
1. Download this repository using **Code → Download ZIP** and extract it. Open a terminal in the extracted repository folder and run `python scripts/build_blender_addon.py` (Python 3 required for this packaging step). This creates `downloads/MW4-Blender-Animation-R21.zip`. Alternatively, without Python, ZIP the **`blender/io_scene_mw4anim` folder itself**, keeping `io_scene_mw4anim/__init__.py` inside the ZIP. Do not ZIP its contents without the containing folder.
...
R21 shows **Textures · R21** and **Animations · R21**. If an older version remains visible, save your project and restart Blender. R10 and later also refresh cached add-on submodules during installation to address earlier upgrade failures.
...
2. Open **N → MW4 → Animations · R21**; scroll down in the sidebar if needed.
...
**Import from MW4 Installation → Resource type: Map terrain → Category: Maps → Folder: maps/<name>** imports supported terrain grids with packed base textures. Frame the selected terrain using **Numpad .**. Tested on URBAN01/02/05 in Blender 5.1. This does not import mission placement, vegetation, water effects, or enable native map export. See the [map import guide](docs/MAP_IMPORT.md) for the exact scope and diagnostics.

## R20: Alpine and mixed terrain records

Fixed Alpine's **Unsupported terrain shape class** failure: culture shape records can coexist with terrain meshes and are now omitted explicitly instead of aborting the map. Alpine imports 9 terrain meshes and 9 packed textures. Culture and water remain outside the terrain-only import scope; omission counts appear in the sidebar and import report. Install R20, restart Blender, and reimport the map using **Resource type: Map terrain**. See [map import details](docs/MAP_IMPORT.md).

## R21: full terrain textures

**Full close-view terrain textures** is enabled by default for map import. R21 assembles the map’s original `composttexture` layers into packed 2048×2048 zone textures for the supplied maps. To upgrade an existing terrain import, select its parent or a child mesh and use **Load / Reload Textures from MW4**. Full composition takes longer and uses more memory/storage than far textures. See the [full terrain texture guide](docs/COMPOST_TEXTURES.md).
