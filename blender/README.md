# MW4 Blender Animation Tools — R12

**Blender 5.1+; add-on version 0.12.0.** Tested with Blender 5.1.0 and 5.2.0. Future Blender versions have not all been tested.
...
R12 shows **Textures · R12** and **Animations · R12**. If an older version remains visible, save your project and restart Blender. R10 and later also refresh cached add-on submodules during installation to address earlier upgrade failures.
...
4. Leave **Import animation Actions** enabled. R12 defaults it to enabled when opening the model-selection dialog.
...
2. Open **N → MW4 → Animations · R12**; scroll down in the sidebar if needed.
...
| Upgrade error mentioning `register_properties` | Install R12; save and restart Blender if an older add-on remains loaded. |
