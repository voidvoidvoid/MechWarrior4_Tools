...
for module_name in ('codec', 'helm', 'archives', 'embedded', 'hierarchy', 'erf',
                    'rig', 'animscript', 'meshes', 'textures', 'animation_ui', 'resource_browser', 'terrain', 'compost', 'game_import', 'erf_write', 'mesh_export', 'assets'):
...
class MW4ANIM_PT_tools(bpy.types.Panel):
    ...
    def draw(self,context):
        ...
        layout.operator(assets.MW4ANIM_OT_erf_import.bl_idname,text='Import standalone ERF (.erf)')
        ...
        if texture_target and texture_target.get('mw4_map'):
            ...
            layout.label(text=texture_target.get('mw4_terrain_texture_status','Baked far textures only'))
        ...
        if root and 'mw4_hierarchy' in root:
            ...
            box.operator(mesh_export.MW4ANIM_OT_export_erfs.bl_idname,text='Export asset ERFs (.zip)')
...
def menu_import(self,context):
    ...
    self.layout.operator(assets.MW4ANIM_OT_erf_import.bl_idname,text='MechWarrior 4 Geometry (.erf)')
...
CLASSES = assets.CLASSES + mesh_export.CLASSES + animation_ui.CLASSES + textures.CLASSES + meshes.CLASSES + game_import.CLASSES + rig.CLASSES + (MW4ANIM_OT_import,MW4ANIM_OT_export,MW4ANIM_PT_tools)
