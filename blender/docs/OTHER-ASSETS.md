...
R18 automatically selects the available resource type when a category/folder contains only ERFs or only hierarchies. Explicit type changes and text searches can still yield no matches, with a count of matches in the other type shown when applicable. `vehicle_hangar2` is verified as a geometry-only folder in the supplied archives. Real `satelite_control` and hangar material loading passed; unresolved references now appear by name in operator messages.
