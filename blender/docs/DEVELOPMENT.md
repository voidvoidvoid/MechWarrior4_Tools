...
R17 adds `resource_browser.py` (pure path grouping/filtering). `tests/test_resource_filters.py` drives Blender RNA filter callbacks, including empty results and stable enum identifiers. `tests/test_static_textures.py [archive-directory [report.json]]` checks qualified paths, unique/ambiguous basename resolution, packed static materials, and optionally the external archive's bdmct1 image and jump-cradle ERF.

R18: `tests/test_atlas_and_buildings.py archive-directory report.json` exercises the external Atlas torso size mismatch, corruption rejection, byte-identical no-op ERF export, corrected edited-size serialization, satellite-control texture assignment and both vehicle-hangar ERFs. Use an external directory containing compatible core.mw4, props.mw4 and textures.mw4. Numeric results are in `validation/r18-atlas-buildings.json`; assets are not bundled.
